...
def transform_matrix(game_board_original, move_dir):
    ...
    # Compression part
    if move_dir == "a" or move_dir == "A":
        game_board = move_left(game_board)

    elif move_dir == "d" or move_dir == "D":
    ...
    return game_board, check_result


def main():
    """
    Main function is the "heart" of the game.
    ...
    while gameplay:
        if is_init:
            game_matrix = place_new(game_matrix)
        ...
        user_input = input()
        # Single game step
        game_matrix, success = transform_matrix(game_matrix, user_input)
        print(np.matrix(game_matrix))

        if win_check(game_matrix):
